...
- `js/world-house-logic.js` — `ROOF_STYLES` / `WALL_STYLES` / `WINDOW_STYLES` / `SIDES`, new house fields `roofStyle`, `roofDir`, `chimney`, `wallStyle`, `windowStyle`, `windows` (`null` = auto, else `[{ side, pos }]` ≤ 8, same-side windows two cells apart), `windowSpan` / `windowBlocked` / `openingsToCells` / `autoWindowCentres` / `canPlaceWindow` — all in edge-offset units along a wall so the editor and engine share one door test.
