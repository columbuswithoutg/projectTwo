...
- Keeper indicator: every unlocked house carries a floating tag over its roof (`🔑 <keeper> · <stay>` plus `you <stay>` when the viewer has time there, `🔑 You` on your own, `🔑 Unclaimed` otherwise; hidden beyond 80 u) — `Playground3D.setHouseKeepers` + `_tickHUD` placement, `.pg3d-keeper-tag`. `GET /houses` also returns `mine` (the caller's stay per island) and the view polls it every 30 s.
