...
const WorldView = (() => {
  ...
  // The panel lives in js/house-editor.js; this passes it the keeper hooks.
  function _openHouseEditor(projectId) {
    if (_houseEditorClose) { _houseEditorClose(); }
    const close = HouseEditor.open({
      projectId,
      title: _projectTitle(projectId),
      unit: 'house',
      subtitle: 'You keep this house (longest stay). Everyone in the world sees what you save. While this panel is open the camera circles your house from outside, so every change shows as you make it. The door stays where the road meets the house.',
      doorTitle: 'Door — fixed by the road',
      doorToast: 'The door is fixed — it sits where the road comes in.',
      maxProps: _maxPropsCap,
      saveUrl: `${API}/world/houses/${encodeURIComponent(projectId)}`,
      savedToast: 'House saved — everyone can see it now.',
      onSaved: (house, data) => { if (data.keeper) _houseKeepers[projectId] = data.keeper; _refreshHouseHud(); },
      onSaveFailed: (data) => { if (data && data.keeper) { _houseKeepers[projectId] = data.keeper; _refreshHouseHud(); } },
      onClose: () => { _houseEditorClose = null; _houseEditing = null; }
    });
    if (!close) return;
    _houseEditing = projectId;
    _houseEditorClose = close;
  }
